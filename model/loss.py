...
def get_self_critical_reward(greedy_sample, sample, gts_batch, scorers, vocab, skip_start_token=False, keep_tokens=False):
    batch_size = sample.size(0)
    max_len = sample.size(1)
    scorer = scorers['CIDEr-D']

    gts = dict(enumerate(gts_batch + gts_batch))
    ...
    return np.repeat(scores[:, np.newaxis], max_len, 1)


def create_mask(sample, sample_log_probs, sample_max_len, vocab):
    mask = torch.zeros(sample_log_probs.size()).float().to(device)
...
class SelfCriticalWithTokenPenaltyLoss(nn.Module):
    ...
    def forward(self, sample, sample_log_probs, greedy_sample, gts_batch, scorers, vocab):
        ...
        reward = get_self_critical_reward(greedy_sample, sample, gts_batch, scorers, vocab, skip_start_token=True, keep_tokens=False)
